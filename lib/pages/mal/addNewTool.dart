...
class AddNewToolMal extends StatefulWidget {
  const AddNewToolMal({Key? key}) : super(key: key);

  @override
  _AAddNewToolMalState createState() => _AAddNewToolMalState();
}

class _AAddNewToolMalState extends State<AddNewToolMal> {
    ...
    return Scaffold(
      ...
      body: Form(
        child: ListView(
          ...
          children: [
            Align(child: Text("നിങ്ങളുടെ ഉപകരണം ചേർക്കുക",style: TextStyle(color: Colors.green,fontSize: 30,fontWeight: FontWeight.bold),),alignment: Alignment.centerLeft,),
            SizedBox(height: 40,),
            Container(
            ...
            TextButton(onPressed: ()async{
              ...
              if(name.text.length!=0 && price.text.length!=0 && place.text.length!=0 && district!="ജില്ല തിരഞ്ഞെടുക്കുക" && path.length!=0){
                showLoading(context);
                var result=await addService.addTool(name: name.text, price: price.text, place: place.text, district: district, phone: phone, path: path);
                if(result=="done"){
