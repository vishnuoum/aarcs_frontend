...
class RentMal extends StatefulWidget {
  const RentMal({Key? key}) : super(key: key);

  @override
  _RentMalState createState() => _RentMalState();
}

class _RentMalState extends State<RentMal> {
  ...
  void init({String query=""})async{
    result=await listService.getTools(query: query);
    if(result!="error"){
      setState(() {
        loading=false;
        txt="Loading";
    ...
    return WillPopScope(
        ...
        child: Scaffold(
          appBar: AppBar(
              ...
              ):Text("ഉപകരണം വടകയ്ക്",style: TextStyle(color: Colors.green),),
              actions: !isSearch?[
              ...
              PopupMenuButton(itemBuilder: (context) {
                return List.generate(2, (index) {
                  var options=["ചേർക്കുക","നിങ്ങളുടെ ഉപകരണങ്ങൾ"];
                  ...
                  onSelected:(index)async{
                    switch(index){
                      case 0:{
                        await Navigator.pushNamed(context,"/addNewToolMal");
                        setState(() {
                          loading=true;
                          result=[];
                        });
                      ...
                      case 1:{
                        await Navigator.pushNamed(context, "/userToolsMal",);
                        setState(() {
                          loading=true;
                          result=[];
                        });
