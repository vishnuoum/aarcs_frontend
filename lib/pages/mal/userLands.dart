...
class UserLandsMal extends StatefulWidget {
  const UserLandsMal({Key? key}) : super(key: key);

  @override
  _UserLandsMalState createState() => _UserLandsMalState();
}

class _UserLandsMalState extends State<UserLandsMal> {
  bool isSearch=false;
  ...
  void init({String query=""})async{
    ...
    result=await listService.getUserLands(phone: phone,query: query);
    print(result);
    if(result!="error"){
      setState(() {
        loading=false;
      ...
      return StatefulBuilder(builder: (BuildContext context,setState){
        return Container(
          child: ListView(
            ...
            children:[
              SizedBox(height: 60,),
              Align(child: Text("തിരുത്തുക",style: TextStyle(color: Colors.green,fontSize: 30,fontWeight: FontWeight.bold),),alignment: Alignment.centerLeft,),
              SizedBox(height: 40,),
              Text("ഭൂമിയുടെ പേര്"),
              Container(
              ...
              TextButton(onPressed: ()async{
                showLoading(context);
                if(name.text.length!=0 && price.text.length!=0 && place.text.length!=0 && district!="ജില്ല തിരഞ്ഞെടുക്കുക"){
                  var res=await listService.editUserLand(name:name.text,price: price.text, district: district,id:id,place: place.text);
                  if(res=="done"){
              ...
              }, child: Text("അപ്ഡേറ്റ്",style: TextStyle(fontSize: 17),),style: TextButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),backgroundColor: Colors.green,primary: Colors.white,padding: EdgeInsets.all(18)),),
              SizedBox(height: 15,),
    ...
    return WillPopScope(
        ...
        child: Scaffold(
          appBar: AppBar(
              ...
              ):Text("നിങ്ങളുടെ ഭൂമി",style: TextStyle(color: Colors.green),),
              actions: !isSearch?[
